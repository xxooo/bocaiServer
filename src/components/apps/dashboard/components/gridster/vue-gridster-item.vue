...
export default {
  ...
  data() {
    return {
      row: this.item.row,
      col: this.item.col,
      sizeX: this.item.sizeX || 2, //this.gridster.defaultSizeX,
      sizeY: this.item.sizeY || 2, //this.gridster.defaultSizeY,
  ...
  methods: {
    init() {
      ...
      const updateDraggable = function() {
        draggable.toggle(that.gridster.draggable && that.gridster.draggable.enabled);
      };
      updateDraggable();

      const resizable = new GridsterResizable(this);
      const updateResizable = function() {
        resizable.toggle(that.gridster.resizable && that.gridster.resizable.enabled);
      };
      updateResizable();
    ...
    sizeChanged() {
      //window.console && console.log('sizeChanged')
    ...
    positionChanged() {
      // window.console && console.log('positionChanged', this.row, this.col)
      ...
      this.$emit('gridster-position-changed', this);
    },
    ...
    setElementSizeY() {
      if (this.gridster && this.gridster.isMobile && !this.gridster.saveGridItemCalculatedHeightInMobile) {
        this.$el.style.height = null;
      } else {
        this.$el.style.height = (this.sizeY * this.gridster.curRowHeight - this.gridster.margins[0]) + 'px';
        this.item.height = this.sizeY * this.gridster.curRowHeight - this.gridster.margins[0];
      }
    },
    setElementSizeX() {
      if (this.gridster && this.gridster.isMobile) {
        this.$el.style.width = null;
      } else {
        this.$el.style.width = (this.sizeX * this.gridster.curColWidth - this.gridster.margins[1]) + 'px';
        this.item.width = this.sizeX * this.gridster.curColWidth - this.gridster.margins[1];
      }
    }
  },
  mounted() {
    this.gridster = this.$parent;
    if (this.gridster && this.gridster.isLoaded) {
      this.init();
    } else {
