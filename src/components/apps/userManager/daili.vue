  ...
  <div id="youxishezhi" class="content-main">
    ...
    <div class="nav">
      ...
      <el-breadcrumb separator="/">
        ...
        <el-breadcrumb-item>代理</el-breadcrumb-item>
      </el-breadcrumb>
    </div>
    <div class="nav">
      <div class="btn-ground">
        ...
        <template v-if="[3,4].findIndex((n) => n==ruleId)>-1">
          总代理 :
          <el-select v-model="zongdluclass" @change="changezongdl" placeholder="请选择" size="mini" style="width:120px;">
            <el-option v-for="(item,index) in gudongList" :value="item.id" :key="item.id" :label="item.username"></el-option> 
          </el-select>
        </template>
        总代理帐号 :
...
export default {
  ...
  methods: {
    ...
    addgudong() {
      this.$router.push({name:'adddaili'});
    },
    ...
    updateuser(item) {
      ...
      this.$router.push({name:'updatedaili'});
    },
    hasitem(item,num) {
    ...
    async childUser() {
      ...
      console.log('zongdluclass',this.zongdluclass);

      let res = await this.$get(`${window.url}/admin/auser/userList?ruleId=6&status=`+this.shuaixuanNum+`&username=`+this.gudongAccout+`&pid=`+this.zongdluclass+`&currentPage=`+this.currentPage+`&pageSize=10`);
