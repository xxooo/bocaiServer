  ...
  <div id="youxishezhi" class="content-main duboBodyClass">
    ...
    <div>
      <div id="submenu">
        ...
        >
          <el-menu-item
            v-for="(item,index) in bocaiCategoryList"
            :key="index"
            :index="item.name"
            @click="bocaiCategoryIdMenu(item)"
    ...
    <div class="portlet">
      <div class="bet_box">
        <div class="orders oodsBodyDiv">
                ...
                <table>
                  ...
                  <tr>
                    <template v-for="(item,index) in longhuhe_lmp.list" v-if="index*1 < 4">
                      <td
                        class="tdLeft ordersTdOver"
                        width="8%"
                        :class="'longhuhe_lmp'+item.oddsId"
                      >{{item.oddsName}}</td>
                      <td
                        class="tdRight selectTd"
                        :class="'longhuhe_lmp'+item.oddsId"
                        @click="selectTd(item,'longhuhe_lmp')"
                      >
                        <ul>
                          <li>
                            <i
                  ...
                  <table>
                    ...
                    <tr v-for="(item,index) in itemPa.list">
                      <td
                        class="tdLeft ordersTdOver"
                        width="8%"
                        :class="'yiwuqiu_lmp'+item.oddsId"
                      >{{item.oddsName}}</td>
                      <td
                        class="tdRight selectTd"
                        :class="'yiwuqiu_lmp'+item.oddsId"
                        @click="selectTd(item,'yiwuqiu_lmp')"
                      >
                        <ul>
                          <li>
                            <i
                              class="iconfont icon-jian"
                              :class="'yiwuqiu_lmp'+item.oddsId"
                              @click="orderTd(item,'yiwuqiu_lmp','jian')"
                            ></i>
                            <input class="odds-font" type="text" v-model="item.odds" />
                            <i
                              class="iconfont icon-jia1"
                              :class="'yiwuqiu_lmp'+item.oddsId"
                              @click="orderTd(item,'yiwuqiu_lmp','add')"
                            ></i>
                          </li>
                        </ul>
                      </td>
                  ...
                  <tr>
                    <template v-for="(item,index) in itemPa.list">
                      <td
                        class="tdLeft ordersTdOver"
                        width="8%"
                        :class="'qianhousan_lmp'+item.oddsId"
                      >{{item.oddsName}}</td>
                      <td
                        class="tdRight selectTd"
                        :class="'qianhousan_lmp'+item.oddsId"
                        @click="selectTd(item,'qianhousan_lmp')"
                      >
                        <ul>
                          <li>
                            <i
                              class="iconfont icon-jian"
                              :class="'qianhousan_lmp'+item.oddsId"
                              @click="orderTd(item,'qianhousan_lmp','jian')"
                            ></i>
                            <input class="odds-font" type="text" v-model="item.odds" />
                            <i
                              class="iconfont icon-jia1"
                              :class="'qianhousan_lmp'+item.oddsId"
                              @click="orderTd(item,'qianhousan_lmp','add')"
                            ></i>
                          </li>
                        </ul>
                      </td>
              ...
              <div class="qiu15_body">
                <div class="nball" v-for="(item_yiwu,index_yiwu) in oddsList">
                  <div class="order-table">
                    <table>
                      <tr>
                        <th colspan="2">{{item_yiwu.name}}</th>
                      </tr>
                      <tr v-for="(item,index) in item_yiwu.list">
                        <td class="oddsNtd tdLeft" :class="'item_yiwu'+item.oddsId">
                          <template v-if="isNaN(item.oddsName*1)">{{item.oddsName}}</template>
                          <template v-else>
                            <div class="ball-icon">{{item.oddsName}}</div>
                          </template>
                        </td>
                        <td
                          class="oddsUltd selectTd"
                          :class="'item_yiwu'+item.oddsId"
                          @click="selectTd(item,'item_yiwu')"
                        >
                          <ul>
                            <li>
                              <i
                                class="iconfont icon-jian"
                                :class="'index_yiwu'+item.oddsId"
                                @click="orderTd(item,'index_yiwu','jian')"
                              ></i>
                              <input class="odds-font" type="text" v-model="item.odds" />
                              <i
                                class="iconfont icon-jia1"
                                :class="'index_yiwu'+item.oddsId"
                                @click="orderTd(item,'index_yiwu','add')"
                              ></i>
                            </li>
                          </ul>
                        </td>
          ...
          <template
            v-if="['二字','一字','三字','二定位','三定位','组选三','组选六','跨度'].findIndex((n) => n == showOdds)>-1"
          >
              ...
              <table class="title">
                <tr>
                  <th
                    ...
                    class="pointerDom yiziThAct"
                    :class="['shishiZi'+index,index == '0' ? 'active' : '']"
                    @click="shishiZiGet(item,index)"
                  >{{item.name}}</th>
                </tr>
              </table>
              ...
              <table>
                ...
                <tr v-for="(itemPa,index) in shishiZiDatasList">
                  <template v-for="(item,index) in itemPa">
                    ...
                    <td
                      class="pointerDom selectTd"
                      :class="'item_yizi'+item.oddsId"
                      @click="selectTd(item,'item_yizi')"
                    >
                      <ul>
                        <li>
                          <i
                            class="iconfont icon-jian"
                            :class="'item_yizi'+item.oddsId"
                            @click="orderTd(item,'item_yizi','jian')"
                          ></i>
                          <input class="odds-font" type="text" v-model="item.odds" />
                          <i
                            class="iconfont icon-jia1"
                            :class="'item_yizi'+item.oddsId"
                            @click="orderTd(item,'item_yizi','add')"
                          ></i>
                        </li>
                      </ul>
                    </td>
                ...
                <table>
                  ...
                  <tr v-for="(itemPa,index) in oddsList">
                    ...
                    <template v-for="(item,index) in itemPa.list">
                      ...
                      <td
                        class="pointerDom selectTd"
                        :class="'item_heshu'+item.oddsId"
                        @click="selectTd(item,'item_heshu')"
                      >
                        <ul>
                          <li>
                            <i
                              class="iconfont icon-jian"
                              :class="'item_heshu'+item.oddsId"
                              @click="orderTd(item,'item_heshu','jian')"
                            ></i>
                            <input class="odds-font" type="text" v-model="item.odds" />
                            <i
                              class="iconfont icon-jia1"
                              :class="'item_heshu'+item.oddsId"
                              @click="orderTd(item,'item_heshu','add')"
                            ></i>
                          </li>
                        </ul>
                      </td>
  ...
  methods: {
    async getoddsCategory() {
      let res = await this.$get(
        `${window.url}/admin/bocai/oddsCategory?bocaiTypeId=` +
          this.curBocaiTypeId +
          `&isBase=` +
          this.isBase
      );
    ...
    async bocaiCategoryIdSub() {
      ...
      await that
        .$get(
          `${window.url}/admin/bocai/odds?bocaiCategoryId=` +
            this.bocaiCategory.id +
            `&isBase=` +
            this.isBase +
            `&bocaiTypeId=` +
            this.curBocaiTypeId
    ...
    shuaiXuanDatas(dataList) {
      if (this.showOdds == "两面盘") {
        ...
        for (let m in dataList) {
          ...
          if (
            ["前三", "中三", "后三"].findIndex(n => n == dataList[m].name) > -1
          ) {
    ...
    async savebet() {
      ...
      await that
        .$post(`${window.url}/admin/bocai/oddsSub`, objdata)
        .then(res => {
          that.$handelResponse(res, result => {
            loading.close();
            if (result.code === 200) {
