<template>
  <div id="youxishezhi" class="content-main">
    ...
    <div class="nav">
      ...
      <span class="mgr10">结算状态：{{baobiaoQinfo.cuserOrderStatus == 1 ? '已结算' : '未结算'}}</span>
      <span class="mgr10">注单类型：正常注单</span>
    </div>
    ...
    <div class="portlet">
          ...
          <tr v-for="aUserReport in aUserPage">
            ...
            <td v-if="ruleId == 6">
              <a
                ...
                v-on:click="getreportCensus(aUserReport.userId,aUserReport.userName)"
              >{{aUserReport.betMoney}}</a>
            </td>
            ...
            <td v-else>
              <a
                ...
                v-on:click="reportList(aUserReport.userId,aUserReport.userName)"
              >{{aUserReport.betMoney}}</a>
            </td>
      ...
      <div class="tab" v-if="ruleId == 6 && showBocaiList">
        <table>
          ...
          <tr v-for="item in censusList" class="po hoverTr" @click="cUserOrderList(item)">
            ...
            <td>{{item.dewaterSum}}</td>
            <td>{{item.winnerMoneySum}}</td>
          </tr>
        </table>
      </div>
...
export default {
  ...
  methods: {
    async getreportCensus(userId, name) {
      this.aUserReportUserId = userId;
      this.aUserReportUserName = name;
      this.showBocaiList = true;

      let url = "admin/report/reportCensus?";

      if (
        null != this.baobiaoQinfo &&
        null != this.baobiaoQinfo.reportType &&
        "" != this.baobiaoQinfo.reportType
      ) {
        url = url + "&reportType=" + this.baobiaoQinfo.reportType;
      }
      if (
        null != this.baobiaoQinfo &&
        null != this.baobiaoQinfo.cuserOrderStatus &&
        "" != this.baobiaoQinfo.cuserOrderStatus
      ) {
      ...
      if (
        this.baobiaoQinfo.timesvalue &&
        this.baobiaoQinfo.timesvalue.length != 0
      ) {
    ...
    async getbaobiaoList(userId, type) {
      ...
      if (
        null != this.baobiaoQinfo.bocaiTypeId &&
        "" != this.baobiaoQinfo.bocaiTypeId
      ) {
